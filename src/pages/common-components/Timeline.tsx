...
const Timeline = ({ mandateCode }) => {
  ...
  const getData = (mandateCode) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASEURL}${process.env.REACT_APP_BASE_EXTENTION}/api/Workflow/GetTimeLines?mandateId=${mandateCode}`
      )
      .then((response) => {
  ...
  let columnDefs = [
    ...
    {    
      ...
      resizable: true,
      cellRenderer: (e: any) => {
        return moment.utc(moment.duration(moment(e?.data?.createdon).diff(moment(e?.data?.taskCreatedOn))).asMilliseconds()).format('HH:mm:ss');
      },
      width: 100,
      minWidth: 100,
      cellStyle: { fontSize: "12px" },
    },
    {
      ...
      resizable: true,
      cellRenderer: (e: any) => {
        return <><Tooltip title="send mail"><MdOutgoingMail size={16} style={{cursor :"pointer"}} onClick={() => {setOpenConfirmDialog(!openConfirmDialog); setSendMailId(e?.data?.id)}} /></Tooltip>
   ...
   const sendMail = () => {
    axios
      .post(
        `${process.env.REACT_APP_BASEURL}${process.env.REACT_APP_BASE_EXTENTION}/api/Workflow/SendPendingEmail?HistoryId=${sendMailId || 0}`, {}
      )
      .then((response) => {
        if(!response){
      ...
      <div style={{ width: "71vw" }}>
        ...
        >
          <Grid
            ...
            alignSelf="end"
          >
            <Button size='small' style={primaryButtonSm} sx={{ color: "#fff", fontSize: "12px", marginRight: "10px" }} onClick={() => togglePanel(!showPanel)}>{showPanel ? <span>Grid </span> : <span> Timeline </span>}</Button>
          ...
          {showPanel ? <div>{timelineData && timelineData?.length > 0 && (
