...
const ProgressBarCard = ({ icon, title, index, selectedPlanning }) => {
  const state = useSelector((state) => state);
  const ProgressBar = state.siteHomeKpi.planningKpiProgressBar ?? [];
  const parameterName = selectedPlanning[title];
  const [barData, setBarData] = useState(null);
  const [color, setColor] = useState(null);

  useEffect(() => {
    if (Object.values(ProgressBar).length > 0) {
      const filterData = ProgressBar?.find((d) => d?.FilterType === parameterName)
  ...
  }, [ProgressBar, parameterName, index, selectedPlanning])

  const handleCardClick = (type) => {
    if (barData?.KpiType !== parameterName) {
  ...
  return (
    <Card onClick={()=>handleCardClick(barData?.TaskType)} className={barData?.KpiType === parameterName ?  "percentWrapper-disabled":"percentWrapper-false"}>
