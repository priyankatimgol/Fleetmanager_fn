...
import LineChartMultiple from 'pages/Templates/TestTemplate';
...
import { getAreaData, getCountryData } from 'redux/actions/UserSiteMapping';
...
const PurchaseRequisition = () => {
  ...
  const fetchGraphData = async (params?: any) => {
    ...
    let ApiPara = `getPurchaseRequisitionGraphData`;
    if (params) {
      ...
      ApiPara = `getPurchaseRequisitionGraphData?${queryParams}`;
    ...
    try {
      ...
      console.log('resultParser__', resultParser);
      ...
      const materialData = resultParser?.filter(
        (item) => item?.KPIType === 'Top 10 Material',
      );
  ...
  console.log('searchVal__', GraphColor);
  ...
  const handleExportToExcel = () => {
    if (gridApi) {
      gridApi.exportDataAsExcel({
        fileName: 'Purchase Requisition Template',
      });
    }
  };
  ...
  const getServerSideDatasource = () => {
    return {
      getRows: async (params) => {
        ...
        let url = `${process.env.REACT_APP_BASEURL}${process.env.REACT_APP_SCM_PORT}/api/scm/getPurchaseRequisition?queryParam=${filterStr}`;
        let colDefObject = `${process.env.REACT_APP_BASEURL}${process.env.REACT_APP_SCM_PORT}/api/scm/GetColumnConfigurations?template=PR`;
        ...
        try {
          ...
          let plantFilterValues;
          if (colDef.some((obj) => obj.DbKey.toLowerCase() === 'plant')) {
            try {
              const response = await fetch(
                'http://fleetmanager.mindnerves.com:10006/api/SCM/getPurchaseOrderDataKpi?reportType=Plant',
              );
              const data = await response.json();
              plantFilterValues = data.map((item) => item.Plant);
            } catch (error) {
                                ...
                                >
                                  ...
                                  <InputLabel
                                    ...
                                    sx={{
                                      ...
                                      display: 'none',
                                    }} // Set background color to white and hide label
                                  ></InputLabel>
                                  <Select
            ...
            <Grid item sm={6} lg={6}>
              <div className='card-inside' style={{ position: 'relative' }}>
                ...
                <BarChartSingle
                  ...
                  GraphColor={GraphColor}
                />
                {/* <LineChartMultiple GraphColor={GraphColor}/> */}
              </div>
            </Grid>
